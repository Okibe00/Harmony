import Drug_Codes from "./drug_code";
...
describe('test the drugCodeGenerator class', () => {
  ...
  test('test instance created', async () => {
    ...
    let [ [row1] ] = await storage.execute('select * from drug_codes INNER JOIN brands ON drug_codes.brand_id=brands.brand_id JOIN manufacturers ON brands.manufacturer_id=manufacturers.manufacturer_id');
